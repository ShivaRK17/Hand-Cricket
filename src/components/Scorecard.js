...
const Scorecard = () => {
  ...
  return (
    <div className='flex flex-col p-3 w-full font-happyMonkey text-black'>
                    <div className="bg-gray-100 p-1 md:p-3 rounded-lg shadow-md w-full">
                        <h2 className="text-xl m-2 font-bold ">SCORECARD</h2>
                            ...
                            {scoretogg ?
                                <tbody>
                                    {uscorecard.map((e, ind) => {
                                        return <tr key={ind} className=''>
                                            <td className="p-1 border border-gray-300"><h6 className='text-xs md:text-base'>{e.name}{ind === ucurrPlayers[0] ? '*' : ""}{' '}{e.score}({e.balls})</h6></td>
                                ...
                                <tbody>
                                    {cscorecard.map((e,ind) => {
                                        return <tr key={ind}>
                                            <td className="p-1 border border-gray-300"><h6 className='text-xs md:text-base'>{e.name}{ind === ccurrPlayers[0] ? '*' : ""}{' '}{e.score}({e.balls})</h6></td>
